...
class AutoencoderKL(pl.LightningModule):
    ...
    def init_from_ckpt(self, path, ignore_keys=list()):
        ...
        print(f"Loaded pretrained Encodec Neural Audio Codec from HuggingFace")

    def encode(self, x):
        ## Encoder forward pass
        inputs = self.processor(raw_audio=x, sampling_rate=self.processor.sampling_rate, return_tensors="pt").to(self.device)
    ...
    def decode(self, encoder_outputs):
        ...
        audio_values = self.encodec.decode(encoder_outputs.audio_codes, encoder_outputs.audio_scales, inputs["padding_mask"])[0]
    ...
    def get_input(self, batch): #TODO CHANGE DATALOADER
